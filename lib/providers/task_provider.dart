...
class TaskProvider extends ChangeNotifier {
  ...
  int get jumlahTerlambat =>
      _semuaTugas.where((t) => t.isTerlambat).length;
  ...
  Future<void> tambahTugas(Task task) async {
    ...
    await _notifService.jadwalkanNotifikasiTugas(task);
    _muatTugas();
  }

  Future<void> editTugas(Task task) async {
    await _notifService.batalkanNotifikasiTugas(task.id);
    await task.save();
    if (!task.isSelesai) {
      await _notifService.jadwalkanNotifikasiTugas(task);
    }
